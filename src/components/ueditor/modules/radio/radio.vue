                    ...
                    <div
                        ...
                        <div class="table-col">
                            <Tooltip content="设为选中" placement="right">
                                <input v-model="dataConfig.labelChecked" type="radio" :value="item.code">
                            </Tooltip>
                        </div>
                        <div class="table-col">
    ...
    methods: {
        ...
        renderDom() {
            ...
            let opts = this.dataConfig,
                ...
                // 只读模式下显示内容
                $readonly = $('<strong v-else-if="isReadonly" class="readonly">{{formObj.'+opts.code +' | radioFilter(formObj, config.'+opts.code+')}}</strong>');
            ...
            M.each(labels, item => {
                ...
                let itemCode = item.code,
                    // 是否被选中
                    isChecked = ( labelChecked === itemCode ) ? 'checked' : '',
                    // 是否存在输入框
                    ...
                    $label = $('\
                        <label class="check-label">\
                            <span class="check-label-ipt">\
                                <input v-model="formObj.'+code+'" type="radio" name="'+code+'" value="'+itemCode+'" '+isChecked+'>\
                                '+item.name+'\
                            </span>\
                        </label>\
                    ');
        ...
        labelBtnClick(type, item, index) {
            ...
            // 如果为删除
                ...
                // 如果当前值等于被选中的值 删除被选中值
                if(itemCode === this.dataConfig.labelChecked) {
                    this.dataConfig.labelChecked = "";
                }
                // 判断是否存在挂载输入框 如果存在 则删除
                let i = M.inArray(this.dataConfig.labelInputs, itemCode);
                if(i !== -1) {
                    this.dataConfig.labelInputs.splice(i, 1)
                }
                // 删除该选项
