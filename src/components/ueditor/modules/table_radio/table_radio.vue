        ...
        renderDom() {
            ...
            M.each(rows, (row, i) => {
                ...
                M.each(cols,(col, j) => {
                    ...
                    // 生成每一行数据
                    rowHtml += '<td>\
                        <input v-if="isEdit" v-model="formObj.'+code+'.'+rowCode+'" type="radio" name="'+rowCode+'" value="'+colCode+'">\
                        <strong v-else-if="isReadonly" class="readonly" v-html="">{{formObj.'+code+'.'+rowCode+' | tableRadioFilter('+colCode+')}}</strong>\
                    </td>';
                });
